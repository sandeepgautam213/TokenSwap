...
const calculateSwap=()=>
{
    ...
    if(tokenSelected==="ABC")
   {
    ...
    return (
        ...
        <nav className ="navbar navbar-expand-lg navbar-light bg-dark rounded-3 ">
            <div className="container-fluid">
            <h1 className="text-light">TokenSwap</h1>
            <label className="badge bg-primary fs-4 rounded">TokenABC Balance: {tokenABalance} Tokens</label>
            <label className="badge bg-primary fs-4 rounded">TokenXYZ Balance: {tokenXBalance} Tokens</label>
            </div>
        </nav>
        <form onSubmit={(evt)=>
            {
            ...
            <div className="d-flex mt-3 input-group">
                <input className="form-control border" placeholder="Amount" name="tkABC" type="number" onChange={(evt)=>{setNumOfTokenA(evt.target.value)}} ></input>
                <button className="btn-danger text-center form-control" type="submit">buy Token ABC</button>
                </div>
        </form>
        ...
        <form onSubmit={(evt)=>
            ...
            <div className="d-flex mt-3 input-group">
                <input className="form-control" type="number" placeholder="Amount" name="tkXYZ" onChange={(evt)=>{setNumOfTokenX(evt.target.value)}} ></input>
                <button className="btn-danger text-center form-control" type="submit">buy Token XYZ</button>
            </div>
        </form>
            <div className="input-group mt-3">
                <select onChange={(evt)=>{
                    setTokenSelected(evt.target.value)
                    }} className="form-select text-center" aria-label="Value">
                    <option value="ABC">switch TokenABC with Token XYZ</option>
                    <option value="XYZ">switch Token XYZ with Token ABC</option>
                </select>
            </div>
            <div className="input-group mt-5">
            <input className="form-control" placeholder="Amount" type="number" onChange={(evt)=>{setSwitchAmount(evt.target.value)}} ></input>
            <button className="btn-warning form-control" onClick={()=>{swapTokens()}}>Swap</button>
            </div>    
            <label className="alert alert-info mt-3">1 ABC = {ratio} XYZ, Fees: {fees}%  Expected to get:{finalAmount}</label>
        </div>
    )
}